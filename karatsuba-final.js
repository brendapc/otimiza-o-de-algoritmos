...
class KaratsubaCalculator {
  ...
  multiply(s, p) {
    // se chegou no final onde cada string só tem 1 digito faz a multiplicação simples e retorna
    ...
    const e = [
      s.length - a[0].length,
      s.length - (a[0].length + (a[1]?.length || 0)),
      0,
    ];
    const f = [
      p.length - b[0].length,
      p.length - (b[0].length + (b[1]?.length || 0)),
      0,
    ];
